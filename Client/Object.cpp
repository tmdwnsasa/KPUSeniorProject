...
void CGameObject::SetMaterial(int nMaterial, CMaterial* pMaterial)
{
	...
	//if (m_pChild)m_pChild->SetMaterial(nMaterial,pMaterial);
	//if (m_pSibling) m_pSibling->SetMaterial(nMaterial, pMaterial);
}
...
{
	CTexture* GhoulTexture = new CTexture(1, RESOURCE_TEXTURE2D, 0);
	GhoulTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"DDS/sample.dds", 0);

	CreateShaderVariables(pd3dDevice, pd3dCommandList);

	CSkinnedAnimationWireFrameShader* GhoulShader = new CSkinnedAnimationWireFrameShader();
	GhoulShader->CreateShader(pd3dDevice, pd3dGraphicsRootSignature);
	GhoulShader->CreateShaderVariables(pd3dDevice, pd3dCommandList);

	CScene::CreateShaderResourceViews(pd3dDevice, GhoulTexture, 15, false);

	CLoadedModelInfo* pAngrybotModel = pModel;

	if (!pAngrybotModel) 
		pAngrybotModel = CGameObject::LoadGeometryAndAnimationFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, "Model/Ghoul@Animations.bin", NULL);
	...
	CMaterial* GhoulMaterial = new CMaterial(1);
	GhoulMaterial->SetTexture(GhoulTexture);
	GhoulMaterial->SetShader(GhoulShader);

	SetMaterial(1,GhoulMaterial);
	//m_pMaterial = GhoulMaterial;
