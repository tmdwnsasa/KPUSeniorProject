...
void CScene::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	CLoadedModelInfo* pAngrybotModel = CGameObject::LoadGeometryAndAnimationFromFile(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, "Model/sample.bin", NULL);
	...
	for (int i = 0; i <25; i++)
	{
		...
		(*(m_vGameObjects.end() - 1))->SetPosition(dis(dre), m_pTerrain->GetHeight(380.0f, 725.0f) + 150, dis(dre));
		//m_vMaterials.push_back(new CMaterial(1));
		//(*(m_vMaterials.end() - 1))->SetTexture(GhoulTexture);
		//(*(m_vMaterials.end() - 1))->SetShader(GhoulShader);
...
ID3D12RootSignature* CScene::CreateGraphicsRootSignature(ID3D12Device* pd3dDevice)
{
	...
	pd3dDescriptorRanges[10].BaseShaderRegister = 0; //t0: ObjectTexture
...
void CScene::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	for (int i = 0; i < m_vGameObjects.size(); i++)
	{
		if (m_vGameObjects[i])
		{
			...
			if (!m_vGameObjects[i]->m_pSkinnedAnimationController)
				m_vGameObjects[i]->UpdateTransform(NULL);

			//m_vMaterials[i]->UpdateShaderVariable(pd3dCommandList);
